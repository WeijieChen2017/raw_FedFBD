...
# Setup logging
log_file = os.path.join(args.output_dir, "fbd_test_tau.log")
...
if not os.path.exists(warehouse_path):
    ...
    if warehouse_files:
        ...
        print(f"No warehouse file found in {args.comm_dir}")
        exit()
...
# Process individual color models
for model_color in final_test_colors:
    ...
    # Get scores from model (inline _get_scores function)
    model.eval()
    ...
    scores = y_score.detach().cpu().numpy()
    all_model_scores.append(scores)
    
    # Evaluate individual model using MedMNIST evaluator (inline _test_model_with_targets function)
    model.eval()
    y_score_eval = torch.tensor([]).to(device)
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            ...
            y_score_eval = torch.cat((y_score_eval, outputs), 0)

    y_score_eval = y_score_eval.detach().cpu().numpy()
    ...
    individual_auc, individual_acc = test_evaluator.evaluate(y_score_eval, None, None)
    ...
    # Prediction diagnostics (inline diagnose_predictions function)
    logger.info(f"=== PREDICTION DIAGNOSTICS for {model_color} ===")
...
if len(all_model_weights) > 0:
    ...
    # Get scores from averaged model
    averaged_model.eval()
    y_score_avg = torch.tensor([]).to(device)
    with torch.no_grad():
        for inputs, _ in test_loader:
            outputs = averaged_model(inputs.to(device))
            ...
            y_score_avg = torch.cat((y_score_avg, outputs), 0)
    
    averaged_scores = y_score_avg.detach().cpu().numpy()
    all_model_scores.append(averaged_scores)
    
    # Evaluate averaged model using MedMNIST evaluator
    averaged_model.eval()
    y_score_avg_eval = torch.tensor([]).to(device)
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            ...
            y_score_avg_eval = torch.cat((y_score_avg_eval, outputs), 0)

    y_score_avg_eval = y_score_avg_eval.detach().cpu().numpy()
    
    test_evaluator_avg = Evaluator(args.experiment_name, 'test', size=args.size)
    avg_auc, avg_acc = test_evaluator_avg.evaluate(y_score_avg_eval, None, None)
    ...
    # Prediction diagnostics for averaging model
    logger.info(f"=== PREDICTION DIAGNOSTICS for averaging ===")
    logger.info(f"Scores shape: {averaged_scores.shape}")
    logger.info(f"Scores range: [{averaged_scores.min():.4f}, {averaged_scores.max():.4f}]")
    logger.info(f"Scores mean: {averaged_scores.mean():.4f}, std: {averaged_scores.std():.4f}")
    
    # Predicted vs true labels
    true_labels = test_dataset.labels.squeeze()
    predicted_labels = np.argmax(averaged_scores, axis=1)
    logger.info(f"Predicted labels range: [{predicted_labels.min()}, {predicted_labels.max()}]")
    logger.info(f"True labels range: [{true_labels.min()}, {true_labels.max()}]")
...
if not all_model_scores:
    ...
    print("No valid model predictions gathered. Aborting.")
    exit()
