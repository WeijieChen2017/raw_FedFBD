...
def main():
    ...
    parser.add_argument("--fedprox_mu", type=float, default=0.1,
                        help="FedProx proximal term coefficient (default: 0.1)")
    ...
    hetero_config = load_hetero_config(f"{args.experiment_name}_tau_prox")
    ...
    print(f"\n6. FEDPROX CONFIGURATION:")
    print(f"   - Algorithm: FedProx (Federated Proximal)")
    print(f"   - Proximal coefficient (μ): {args.fedprox_mu}")
    print(f"   - Proximal term: (μ/2)||w - w_global||²")
    fedprox_status = "Enabled" if args.fedprox_mu > 0 else "Disabled"
    print(f"   - Status: {fedprox_status}")
